...
! --------------------------------------------------------
!
! Radiative source function and O2 Js for 137 layers.
!
...
! L137 pressures 1 (995.367) is > L72 pressure 1 (992.5000).
!
! --------------------------------------------------------
...
INCLUDE 'netcdf.inc'
 ...
 INTEGER, PARAMETER :: km137 = 137
 REAL, ALLOCATABLE :: levs137(:)
 REAL, ALLOCATABLE :: levsr(:),levs137r(:)
...
! 137-layer mean reference pressures (hPa)
! ---------------------------------------
 ALLOCATE(levs137(km137), STAT=status)
 fileName = "L137/137-layer.p"
 OPEN(UNIT=unit,FILE=TRIM(fileName),STATUS="old",FORM="formatted",ACTION="read")
 READ(UNIT,FMT="(A10)") string
 READ(UNIT,FMT="(A10)") string
 READ(UNIT,FMT="(30X,F9.4)") r
...
! ---------------------------------------
 ...
 ALLOCATE(levs137r(km137), STAT=status)
 levs137r(1:km137) = ALOG(levs137(km137:1:-1))
 PRINT *,'----------- ALOG(levsr) ------------'
 PRINT *,levsr
 PRINT *,'---------- ALOG(levs137r) ----------'
 PRINT *,levs137r
...
! --------------------
 fileName = "output/SC.J_20_12_79_137_200_38.nc4"
...
! ---------------------------------------------------------
 ...
 status = NF_DEF_DIM(fid, 'layers', km137, levdim)
 IF(status /= NF_NOERR) THEN
...
! ------
 ...
 status = NF_PUT_VAR(fid, levid, levs137)
 IF(status /= NF_NOERR) THEN
...
PRINT *, nsza,numo3,km,km137,nlam
 ...
 ALLOCATE(x2(numo3,km137), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km137), STAT=status)
 
 DO m = 1,numo3
  x1(1:km) = o3_tab(m,km:1:-1)
  CALL interp(levsr,x1,km,levs137r,xo,km137)
  x2(m,1:km137) = xo(km137:1:-1)
  x2(m,1:3) = o3_tab(m,1)
  IF(m == numo3/3) THEN
   PRINT *,'---------- o3_tab ----------'
   PRINT *,o3_tab(m,1:km)
 ...
 ALLOCATE(x4(nsza,numo3,km137,nlam), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km137), STAT=status)
 ...
 ALLOCATE(x3(nsza,numo3,km137), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km137), STAT=status)
