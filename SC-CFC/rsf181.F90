...
! --------------------------------------------------------
!
! Radiative source function and O2 Js for 181 layers.
!
...
! L181 pressures 1 (995.367) is > L72 pressure 1 (992.5000).
!
! --------------------------------------------------------
...
INCLUDE 'netcdf.inc'
 ...
 INTEGER, PARAMETER :: km181 = 181
 REAL, ALLOCATABLE :: levs181(:)
 REAL, ALLOCATABLE :: levsr(:),levs181r(:)
...
! 181-layer mean reference pressures (hPa)
! ---------------------------------------
 ALLOCATE(levs181(km181), STAT=status)
 fileName = "L181/181-layer.p"
 OPEN(UNIT=unit,FILE=TRIM(fileName),STATUS="old",FORM="formatted",ACTION="read")
 READ(UNIT,FMT="(A10)") string
 READ(UNIT,FMT="(A10)") string
 READ(UNIT,FMT="(30X,F9.4)") r
...
! ---------------------------------------
 ...
 ALLOCATE(levs181r(km181), STAT=status)
 levs181r(1:km181) = ALOG(levs181(km181:1:-1))
 PRINT *,'----------- ALOG(levsr) ------------'
 PRINT *,levsr
 PRINT *,'---------- ALOG(levs181r) ----------'
 PRINT *,levs181r
...
! --------------------
 fileName = "output/SC.J_20_12_79_181_200_38.nc4"
...
! ---------------------------------------------------------
 ...
 status = NF_DEF_DIM(fid, 'layers', km181, levdim)
 IF(status /= NF_NOERR) THEN
...
! ------
 ...
 status = NF_PUT_VAR(fid, levid, levs181)
 IF(status /= NF_NOERR) THEN
...
PRINT *, nsza,numo3,km,km181,nlam
 ...
 ALLOCATE(x2(numo3,km181), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km181), STAT=status)
 
 DO m = 1,numo3
  x1(1:km) = o3_tab(m,km:1:-1)
  CALL interp(levsr,x1,km,levs181r,xo,km181)
  x2(m,1:km181) = xo(km181:1:-1)
  x2(m,1:3) = o3_tab(m,1)
  IF(m == numo3/3) THEN
   PRINT *,'---------- o3_tab ----------'
   PRINT *,o3_tab(m,1:km)
 ...
 ALLOCATE(x4(nsza,numo3,km181,nlam), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km181), STAT=status)
 ...
 ALLOCATE(x3(nsza,numo3,km181), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km181), STAT=status)
