...
! --------------------------------------------------------
!
! Radiative source function and O2 Js for 186 layers.
!
...
! L186 pressures 1 (995.367) is > L72 pressure 1 (992.5000).
!
! --------------------------------------------------------
...
INCLUDE 'netcdf.inc'
 ...
 INTEGER, PARAMETER :: km186 = 186
 REAL, ALLOCATABLE :: levs186(:)
 REAL, ALLOCATABLE :: levsr(:),levs186r(:)
...
! 186-layer mean reference pressures (hPa)
! ---------------------------------------
 ALLOCATE(levs186(km186), STAT=status)
 fileName = "L186/186-layer.p"
 OPEN(UNIT=unit,FILE=TRIM(fileName),STATUS="old",FORM="formatted",ACTION="read")
 READ(UNIT,FMT="(A10)") string
 READ(UNIT,FMT="(A10)") string
 READ(UNIT,FMT="(30X,F9.4)") r
...
! ---------------------------------------
 ...
 ALLOCATE(levs186r(km186), STAT=status)
 levs186r(1:km186) = ALOG(levs186(km186:1:-1))
 PRINT *,'----------- ALOG(levsr) ------------'
 PRINT *,levsr
 PRINT *,'---------- ALOG(levs186r) ----------'
 PRINT *,levs186r
...
! --------------------
 fileName = "output/SC.J_20_12_79_186_200_38.nc4"
...
! ---------------------------------------------------------
 ...
 status = NF_DEF_DIM(fid, 'layers', km186, levdim)
 IF(status /= NF_NOERR) THEN
...
! ----------
 ...
 string = "NetCDF version created on 2019-Mar-29 by matthew.thompson@nasa.gov SSAI/GMAO"
 i = LEN_TRIM(string)
...
! ------
 ...
 status = NF_PUT_VAR(fid, levid, levs186)
 IF(status /= NF_NOERR) THEN
...
PRINT *, nsza,numo3,km,km186,nlam
 ...
 ALLOCATE(x2(numo3,km186), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km186), STAT=status)
 
 DO m = 1,numo3
  x1(1:km) = o3_tab(m,km:1:-1)
  CALL interp(levsr,x1,km,levs186r,xo,km186)
  x2(m,1:km186) = xo(km186:1:-1)
  x2(m,1:3) = o3_tab(m,1)
  IF(m == numo3/3) THEN
   PRINT *,'---------- o3_tab ----------'
   PRINT *,o3_tab(m,1:km)
 ...
 ALLOCATE(x4(nsza,numo3,km186,nlam), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km186), STAT=status)
 ...
 ALLOCATE(x3(nsza,numo3,km186), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km186), STAT=status)
