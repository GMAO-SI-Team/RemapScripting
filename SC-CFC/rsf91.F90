...
! --------------------------------------------------------
!
! Radiative source function and O2 Js for 91 layers.
!
...
! L91 pressures 1 (995.367) is > L72 pressure 1 (992.5000).
!
! --------------------------------------------------------
...
INCLUDE 'netcdf.inc'
 ...
 INTEGER, PARAMETER :: km91 = 91
 REAL, ALLOCATABLE :: levs91(:)
 REAL, ALLOCATABLE :: levsr(:),levs91r(:)
...
! 91-layer mean reference pressures (hPa)
! ---------------------------------------
 ALLOCATE(levs91(km91), STAT=status)
 fileName = "L91/91-layer.p"
 OPEN(UNIT=unit,FILE=TRIM(fileName),STATUS="old",FORM="formatted",ACTION="read")
 READ(UNIT,FMT="(A10)") string
 READ(UNIT,FMT="(A10)") string
 READ(UNIT,FMT="(30X,F9.4)") r
...
! ---------------------------------------
 ...
 ALLOCATE(levs91r(km91), STAT=status)
 levs91r(1:km91) = ALOG(levs91(km91:1:-1))
 PRINT *,'----------- ALOG(levsr) ------------'
 PRINT *,levsr
 PRINT *,'---------- ALOG(levs91r) ----------'
 PRINT *,levs91r
...
! --------------------
 fileName = "output/SC.J_20_12_79_91_200_38.nc4"
...
! ---------------------------------------------------------
 ...
 status = NF_DEF_DIM(fid, 'layers', km91, levdim)
 IF(status /= NF_NOERR) THEN
...
! ------
 ...
 status = NF_PUT_VAR(fid, levid, levs91)
 IF(status /= NF_NOERR) THEN
...
PRINT *, nsza,numo3,km,km91,nlam
 ...
 ALLOCATE(x2(numo3,km91), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km91), STAT=status)
 
 DO m = 1,numo3
  x1(1:km) = o3_tab(m,km:1:-1)
  CALL interp(levsr,x1,km,levs91r,xo,km91)
  x2(m,1:km91) = xo(km91:1:-1)
  x2(m,1:3) = o3_tab(m,1)
  IF(m == numo3/3) THEN
   PRINT *,'---------- o3_tab ----------'
   PRINT *,o3_tab(m,1:km)
 ...
 ALLOCATE(x4(nsza,numo3,km91,nlam), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km91), STAT=status)
 ...
 ALLOCATE(x3(nsza,numo3,km91), STAT=status)
 ALLOCATE(x1(km), STAT=status)
 ALLOCATE(xo(km91), STAT=status)
